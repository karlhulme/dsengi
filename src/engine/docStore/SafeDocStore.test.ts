...
Deno.test("A safe doc store passes through values from the underlying doc store.", async () => {
  ...
  assertEquals(await safeDocStore.selectAll("", "", "all", {}), {
    docs: [],
    queryCharge: 0,
  });
  assertEquals(await safeDocStore.selectByDigest("", "", "", {}), {
    docs: [],
    queryCharge: 0,
  });
  assertEquals(await safeDocStore.selectByFilter("", "", {}, "all", {}), {
    docs: [],
    queryCharge: 0,
  });
...
Deno.test("A safe doc store wraps underlying errors.", async () => {
  ...
  await assertRejects(
    () => safeDocStore.selectAll("", "", "all", {}),
    UnexpectedDocStoreError,
  );
  await assertRejects(
    () => safeDocStore.selectByDigest("", "", "", {}),
    UnexpectedDocStoreError,
  );
  await assertRejects(
    () => safeDocStore.selectByFilter("", "", {}, "all", {}),
    UnexpectedDocStoreError,
  );
  await assertRejects(
